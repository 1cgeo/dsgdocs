...
controller.login = async (login, password) => {
  try {
    let user = await db.oneOrNone(
      `
    SELECT id, password FROM doc.usuario WHERE login = $1
    `,
      [login]
    );
    if (user) {
...
controller.createUser = async (name, login, password) => {
  try {
    ...
    await db.any(
      `
      INSERT INTO doc.usuario(nome, login, senha) VALUES($1,$2,$3)
      `,
      [name, login, hash]
    );
...
controller.updateUser = async (id, name, login) => {
  try {
    let result = await db.result(
      `
      UPDATE doc.usuario SET nome =$1, login=$2 WHERE id = $3
      `,
      [name, login, id]
    );
...
controller.updatePassword = async (id, oldpassword, newpassword) => {
  try {
    let user = await db.oneOrNone(
      `
    SELECT senha FROM doc.usuario WHERE id = $1
    `,
      [id]
    );
    if (user) {
      const match = await bcrypt.compare(user.senha, oldpassword);
      if (match) {
        let hashNew = await bcrypt.hash(newpassword, 10);

        let result = await db.result(
          `
          UPDATE doc.usuario SET senha =$1 WHERE id = $2
          `,
          [hashNew, id]
        );
        if (!result.rowCount || result.rowCount < 1) {
