...
controller.get = async () => {
  try {
    let data = await db.any(
      `
      SELECT id, name, description, category_id FROM doc.arquivo
      `
    );
...
controller.update = async (id, name, description, categoryId) => {
  try {
    let result = await db.result(
      `
      UPDATE doc.arquivo SET name =$1, description =$2, category_id =$3 WHERE id = $4
      `,
    ...
    await db.tx(async t => {
      if (!workspaceId) {
        let workspace = await t.one(
          `
          INSERT INTO doc.arquivo(name, description, category_id) VALUES($1,$2,$3) RETURNING id
          `,
      ...
      let version = await t.one(
        `
        INSERT INTO doc.workspace_version(workspace_id, name, version_date, author, workspace_path, accessible)
      ...
      params.forEach(p => {
        queries.push(
          t.none(
            `
            INSERT INTO doc.parameters(workspace_version_id, name)
            VALUES($1, $2)
            `,
            [version.id, p]
          )
